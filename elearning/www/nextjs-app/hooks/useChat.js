...
export const useChat = () => {
  ...
  useEffect(() => {
    if (session?.user?.email) {
      const savedMessages = localStorage.getItem(`chat_messages_${session.user.email}`);
      const savedSessionId = localStorage.getItem(`chat_session_${session.user.email}`);
  ...
  useEffect(() => {
    if (session?.user?.email && messages.length > 0) {
      localStorage.setItem(`chat_messages_${session.user.email}`, JSON.stringify(messages));
    }
  ...
  useEffect(() => {
    if (session?.user?.email && currentSessionId) {
      localStorage.setItem(`chat_session_${session.user.email}`, currentSessionId);
    }
  ...
  const createChatSession = useCallback(async () => {
    ...
    try {
      const response = await fetch('/api/method/elearning.elearning.agents.tutor.create_chat_session', {
        method: 'POST',
        headers: {
  ...
  const sendMessage = useCallback(async (content, attachments = []) => {
    if ((!content.trim() && attachments.length === 0) || !session?.user) return;
    ...
    try {
      ...
      const response = await fetch('/api/method/elearning.elearning.agents.tutor.handle_chat_message', {
        method: 'POST',
        body: formData,
      });
      ...
      console.log('🐛 CHAT DEBUG - Proactive Response:', data.message?.proactive_response);
      
      if (data.message?.proactive_response?.debug_proactive) {
        console.log('🎯 KNOWLEDGE GAP DEBUG:', data.message.proactive_response.debug_proactive);
      }
      ...
      if (data.message?.success && data.message.response) {
        ...
        if (data.message.proactive_response) {
          ...
          if (typeof data.message.proactive_response === 'object' && data.message.proactive_response.type) {
            proactiveMessage.tutoring_data = data.message.proactive_response;
            proactiveMessage.content = data.message.proactive_response.content;
          } else {
  ...
  const handleTutoringButtonClick = useCallback((action, data, buttonText) => {
